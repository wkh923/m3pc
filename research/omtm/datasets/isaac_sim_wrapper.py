...
class MTMEnvWrapper(VecEnv):
    ...
    def align_norm_with_dataset(self, norm_dict: dict):
        self.obs_mean = norm_dict["obs_mean"]
        self.obs_std = norm_dict["obs_std"]
        self.act_mean = norm_dict["act_mean"]
        self.act_std = norm_dict["act_std"]
        self.rew_mean = norm_dict["rew_mean"]
        self.rew_std = norm_dict["rew_std"]
    ...
    def step(self, actions: np.ndarray) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, dict]:
        ...
        dones = (terminated | truncated)[0].cpu().numpy()
