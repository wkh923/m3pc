...
from research.omtm.datasets.mimic_dataset import MIMICDataset
from research.omtm.distributed_utils import DistributedParams, get_distributed_params
from research.omtm.masks import (
    MaskType,
    create_bc_mask,
...
def eval_id(
    model: omtm, env, eval_batch, tokenizer_manager, ratio: int = 1
) -> Dict[str, Any]:
...
def create_eval_logs_states(
    predict_fn: Callable,  # TODO: use model in sample mode, not dist mode
    trajectories: Dict[str, torch.Tensor],
    tokenizer_manager: TokenizerManager,
) -> Dict[str, Any]:
    assert "states" in trajectories
...
def train_one_batch(
    ...
    temperature_loss = model.temperature() * (entropy - model.target_entropy).detach()
    ...
    with torch.no_grad():
        ...
        for k, v in predictions.items():
            ...
            if k != "actions":
                _mse = F.mse_loss(
                    v.to(torch.float32), batch[k].to(torch.float32)
                ).item()
...
def _main(hydra_cfg):
    ...
    dp.local_rank = cfg.local_cuda_rank
    ...
    train_dataset = MIMICDataset(
        sequence_length=cfg.traj_length, max_eval_steps=cfg.eval_steps
    )
    ...
    optimizer = omtm.configure_optimizers(
        model,
    ...
    wandb_enabled = hydra_cfg.wandb.enable

    print("=== Wandb enabled: ", wandb_enabled)
    ...
    if wandb_enabled and wandb_cfg_log.resume:
    ...
    if wandb_enabled and wandb_logger.enable and wandb.run.resumed:
    ...
    if hydra_cfg.checkpoints.use_checkpoint:
        ckpt_path = hydra_cfg.checkpoints.checkpoint_path
        ckpt = torch.load(ckpt_path, map_location=cfg.device)
        logger.info(f"Using checkpoint: {ckpt_path}")
        step = ckpt["step"]
        model.load_state_dict(ckpt["model"])
        optimizer.load_state_dict(ckpt["optimizer"])
        eval_max = ckpt["eval_max"]
        logger.info(f"CHECKPOINTS LOADED: starting from step={step}")
    ...
    while True:
        ...
        if random.randint(0, cfg.log_every) == 0 and wandb_enabled:
...
@hydra.main(config_path=".", config_name="config_mimic", version_base="1.1")
